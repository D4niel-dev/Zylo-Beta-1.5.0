# Privacy Policy

**Effective Date:** January 4, 2026  
**Last Updated:** February 13, 2026

## Introduction

Welcome to Zylo. This Privacy Policy explains how Zylo ("we," "us," or "our") collects, uses, discloses, and protects your personal information when you use our platform, including our website, mobile applications, and related services (collectively, the "Service").

By accessing or using Zylo, you agree to the terms of this Privacy Policy. If you do not agree with our practices, please do not use the Service.

---

## 1. Information We Collect

### 1.1 Information You Provide

**Account Registration:**
- Username and usertag
- Email address
- Password (encrypted and hashed)
- Date of birth
- Gender (optional)
- Phone number (optional)
- Profile pictures (avatar and banner)

**User-Generated Content:**
- Messages, posts, and comments
- Media files (images, videos, audio)
- Group and chat data
- Explore feed content

**Communications:**
- Support inquiries and correspondence
- Feedback and survey responses

### 1.2 Information Collected Automatically

**Usage Data:**
- Pages visited and features accessed
- Time spent on the platform
- Interaction patterns (clicks, scrolls, navigation)
- Device information (type, operating system, browser)
- IP address and general location data
- Referral sources

**Technical Data:**
- Cookies and similar tracking technologies
- Session identifiers
- Error logs and diagnostic data

### 1.3 Information from Third Parties

If you choose to sign up or log in through third-party services (Google, GitHub, Discord, Microsoft), we may receive:
- Basic profile information (name, email, profile picture)
- Authentication tokens
- Information permitted by your privacy settings on those platforms

---

## 2. How We Use Your Information

We use collected information for the following purposes:

**Service Delivery:**
- Create and manage your account
- Authenticate and verify your identity
- Enable communication features (messaging, groups, explore feed)
- Store and display user-generated content

**Service Improvement:**
- Analyze usage patterns and trends
- Develop new features and functionality
- Optimize performance and user experience
- Conduct research and analytics

**Communication:**
- Send important service notifications
- Respond to support requests
- Provide updates about new features
- Send marketing communications (with consent)

**Security and Compliance:**
- Detect and prevent fraud, abuse, and security incidents
- Enforce our Terms of Service
- Comply with legal obligations
- Protect rights, property, and safety

---

## 3. Data Sharing and Disclosure

We do not sell your personal information. We may share your information in the following circumstances:

### 3.1 With Your Consent
We share information when you explicitly authorize us to do so.

### 3.2 Service Providers
We work with third-party vendors who assist with:
- Cloud hosting and storage
- Analytics and performance monitoring
- Email delivery
- Customer support tools
- Payment processing (if applicable)

These providers are contractually obligated to protect your data and use it only for specified purposes.

### 3.3 Legal Requirements
We may disclose information when required by law or in response to:
- Valid legal processes (subpoenas, court orders)
- Government or regulatory requests
- National security requirements

### 3.4 Business Transfers
In the event of a merger, acquisition, or sale of assets, your information may be transferred to the acquiring entity.

### 3.5 Public Information
Content you share publicly (profile information, explore posts, public group messages) is visible to other users and may be indexed by search engines.

---

## 4. Data Retention

We retain your personal information for as long as:
- Your account remains active
- Necessary to provide the Service
- Required for legal, regulatory, or security purposes

When you delete your account, we will remove your personal data within 90 days, except where retention is legally required.

---

## 5. Your Rights and Choices

### 5.1 Access and Correction
You can access and update your account information through your profile settings.

### 5.2 Data Portability
You may request a copy of your personal data in a machine-readable format.

### 5.3 Deletion
You can request deletion of your account and associated data by contacting support or using in-app deletion features.

### 5.4 Marketing Opt-Out
You can unsubscribe from marketing emails using the unsubscribe link in each message.

### 5.5 Cookie Management
You can control cookies through your browser settings. Note that disabling cookies may limit functionality.

### 5.6 Do Not Track
Our Service does not currently respond to "Do Not Track" signals.

---

## 6. Security Measures

We implement industry-standard security practices to protect your data:

**Technical Safeguards:**
- Encryption in transit (TLS/SSL)
- Encrypted password storage (bcrypt hashing)
- Secure authentication mechanisms
- Regular security audits and updates

**Organizational Safeguards:**
- Limited access to personal data
- Employee training on data protection
- Incident response procedures

Despite our efforts, no security system is impenetrable. We cannot guarantee absolute security.

---

## 7. Children's Privacy

Zylo is not intended for users under 13 years of age. We do not knowingly collect personal information from children under 13.

If you are a parent or guardian and believe your child has provided us with personal information, please contact us at zylosupp0rt@gmail.com. We will promptly delete such information.

---

## 8. International Data Transfers

Your information may be transferred to and processed in countries other than your country of residence. These countries may have different data protection laws.

When we transfer data internationally, we implement appropriate safeguards, including:
- Standard contractual clauses
- Adequacy decisions by relevant authorities
- Consent where required

---

## 9. Third-Party Links

The Service may contain links to third-party websites or services. We are not responsible for the privacy practices of these third parties. We encourage you to review their privacy policies.

---

## 10. Changes to This Privacy Policy

We may update this Privacy Policy periodically to reflect changes in our practices, technology, or legal requirements.

**Notification of Changes:**
- Updated "Last Updated" date at the top
- Prominent notice on the Service
- Email notification for material changes (where applicable)

Your continued use of the Service after changes constitutes acceptance of the updated Privacy Policy.

---

## 11. Contact Information

If you have questions, concerns, or requests regarding this Privacy Policy or our data practices, please contact us:

**Email:** zylosupp0rt@gmail.com  
**Website:** https://d4niel-dev.github.io/Zylo/  
**GitHub:** https://github.com/D4niel-dev

---

## 12. Regional Provisions

### 12.1 For European Users (GDPR)

If you are located in the European Economic Area (EEA), UK, or Switzerland, you have additional rights under the General Data Protection Regulation (GDPR):

- Right to access your personal data
- Right to rectification of inaccurate data
- Right to erasure ("right to be forgotten")
- Right to restrict processing
- Right to data portability
- Right to object to processing
- Right to withdraw consent
- Right to lodge a complaint with a supervisory authority

**Legal Basis for Processing:**
- Consent for certain processing activities
- Contract performance for service delivery
- Legitimate interests for analytics and improvements
- Legal obligations for compliance

### 12.2 For California Users (CCPA)

California residents have specific rights under the California Consumer Privacy Act (CCPA):

- Right to know what personal information is collected
- Right to know if personal information is sold or disclosed
- Right to opt-out of the sale of personal information (we do not sell data)
- Right to request deletion of personal information
- Right to non-discrimination for exercising privacy rights

To exercise these rights, contact us at zylosupp0rt@gmail.com.

---

## 13. Cookies and Tracking Technologies

### 13.1 Types of Cookies We Use

**Essential Cookies:**
- Authentication and session management
- Security features

**Functional Cookies:**
- User preferences (theme, language)
- Remember me functionality

**Analytics Cookies:**
- Usage statistics and performance metrics
- User behavior analysis

**Marketing Cookies (if applicable):**
- Targeted advertising
- Conversion tracking

### 13.2 Managing Cookies

You can control cookies through:
- Browser settings
- Opt-out tools provided by analytics services
- Platform-specific privacy settings

---

**By using Zylo, you acknowledge that you have read and understood this Privacy Policy.**